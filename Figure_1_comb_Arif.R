...
##Plasma miRNA###
comb = read.csv("comb_data.csv")

length(unique(comb$SDG_ID));unique(comb$SDG_ID)

library(msgl)
fit.cv <- msgl::cv(as.matrix(comb[,3:2084]), comb$Short_histology, fold = 4, alpha = 0.5, lambda = 0.1, use_parallel = TRUE, standardize = FALSE)
fit.cv

fit <- msgl::fit(as.matrix(comb[,3:2084]), comb$Short_histology, alpha = 0.1, lambda = 0.1, standardize = FALSE)
fit
...
p2 = ggplot(comb, aes(x = reorder(Short_histology, `miR.3197_p`, mean), y = `miR.3197_p`, group = Short_histology, color = reorder(Short_histology, -`miR.3197_p`, mean))) + 
...
p3 = ggplot(comb, aes(x = reorder(Short_histology, `miR.451a_p`, mean), y = `miR.451a_p`, group = Short_histology, color = reorder(Short_histology, -`miR.451a_p`, mean))) + 
  geom_boxplot(outlier.shape = NA) + geom_jitter(shape=16, size = 6,position=position_jitter(0.2)) + 
...
p4 = ggplot(comb, aes(x = reorder(Short_histology, `miR.4745.3p_p`, mean), y = `miR.4745.3p_p`, group = Short_histology, color = reorder(Short_histology, -`miR.4745.3p_p`, mean))) + 
  geom_boxplot(outlier.shape = NA) + geom_jitter(shape=16, size = 6,position=position_jitter(0.2)) + 
...
p5 = ggplot(comb, aes(x = reorder(Short_histology, `miR.6126_p`, mean), y = `miR.6126_p`, group = Short_histology, color = reorder(Short_histology, -`miR.6126_p`, mean))) + 
  geom_boxplot(outlier.shape = NA) + geom_jitter(shape=16, size = 6,position=position_jitter(0.2)) + 
...
p6 = ggplot(comb, aes(x = reorder(Short_histology, `miR.6870.3p_p`, mean), y = `miR.6870.3p_p`, group = Short_histology, color = reorder(Short_histology, -`miR.6870.3p_p`, mean))) + 
  geom_boxplot(outlier.shape = NA) + geom_jitter(shape=16, size = 6,position=position_jitter(0.2)) + 
...
require(nnet)
multinom.fit <- multinom(Short_histology ~ `miR.3197_p` + `miR.451a_p` + `miR.4745.3p_p` + `miR.6126_p`, `miR.6870.3p_p`, data = comb)
...
prediction = predict(multinom.fit, newdata = comb, "class")
sum(prediction == comb$Short_histology)/nrow(comb)


result1 = predict(multinom.fit, newdata = comb, type='probs')
library(HandTill2001)
auc(multcap(
  response = factor(comb$Short_histology),
